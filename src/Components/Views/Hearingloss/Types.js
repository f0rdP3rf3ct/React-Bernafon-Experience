...
export class Types extends Component {
    ...
    handleOnClick = (typeSuffix, event) => {
        ...
        // handle styling
        const allLabels = Array.from(document.getElementsByClassName(styles.labelWrapper));

        allLabels.forEach(obj => {
            obj.classList.remove(styles.activelabel);
         });

        const activeRows = Array.from(document.querySelectorAll('[data-belongsto=' + typeSuffix + ']'));
